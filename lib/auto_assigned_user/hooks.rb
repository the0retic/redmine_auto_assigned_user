...
=> [:user, :roles]).first.blank? # If the user is the client, fill the "assigned to" field
		...
		if context[:issue].tracker_id == @settings['tracker_director'].to_i
                	context[:issue].assigned_to_id = users_list[manager_role].first.id unless users_list[manager_role].blank?
		else
			context[:issue].assigned_to_id = users_list[director_role].first.id unless users_list[director_role].blank?
		end
